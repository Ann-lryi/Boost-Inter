...
class MainActivity : AppCompatActivity() {
    ...
    override fun onDestroy() {
        super.onDestroy()
        // NativePacketEngine là singleton toàn app, không tự giải phóng theo vòng
        // đời Activity. Nếu không gỡ callback ở đây, lambda này (giữ tham chiếu
        // tới binding/View của Activity qua closure) sẽ giữ cả Activity đã huỷ
        // trong bộ nhớ chừng nào VPN service còn chạy nền — rò rỉ bộ nhớ kéo dài
        // suốt phiên VPN, không chỉ tới khi Activity bị huỷ.
        NativePacketEngine.onStatsUpdated = null
    }
    ...
    private fun updateUiState(active: Boolean) {
        ...
        if (active) {
            ...
            binding.tvDnsInfo.text = "DNS Race: 4 servers"
    ...
    private fun checkAndroid16Permissions() {
        ...
        val prefs = getSharedPreferences("network_optimizer_prefs", MODE_PRIVATE)
        // Chỉ hỏi 1 lần: bản gốc gọi lại màn hình hệ thống này ở MỌI lần mở app
        // cho tới khi được cấp quyền, kể cả khi user đã chủ động từ chối trước đó —
        // gây phiền vì cứ mở app là bị đẩy sang màn hình cài đặt hệ thống.
